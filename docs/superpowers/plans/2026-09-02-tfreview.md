...
**Goal:** `terraform plan` の結果だけを入力に、config で定義した観点ごとの危険度を判定して PR コメント 1 個と `tfreview:*` ラベルに出す Go CLI（`extract` / `review` / `comment` / `fetch`）と、それを呼ぶ composite GitHub Action を作る。

**Architecture:** サブコマンド間は JSON ファイルで受け渡す。`review` は match（決定的）→ LLM（target ごとに Messages API 1 回、state があれば再利用）→ target 跨ぎ merge → ask fallback → max 集約の順で判定し、`result.json` / `comment.md` / `label.txt` / `state.json` を書く。LLM は `Provider` interface の背後に置き、v1 は anthropic 実装と mock 実装だけを持つ。

**Tech Stack:** Go 1.24、`github.com/spf13/cobra v1.10.2`、`github.com/goccy/go-yaml v1.19.2`、`github.com/anthropics/anthropic-sdk-go v1.69.0`、`github.com/stretchr/testify v1.12.1`。GitHub API は `net/http` で直接叩く（依存を増やさない）。
...
- 単位の呼び名は **target**。config の match 条件は `actions` / `types` / `targets` のみ
- レベルは `none < medium < high < critical`。verdict は `hit` / `miss` / `unverifiable` / `skipped`
- ラベルは `tfreview:none|medium|high|critical|unknown`。色は none `0E8A16` / medium `FBCA04` / high `D93F0B` / critical `B60205` / unknown `0075CA`
- コメントのマーカーは `<!-- tfreview:begin -->` / `<!-- tfreview:end -->`
- 出力言語の既定は `en`。`ja` も同梱する
- LLM の既定 model は `claude-opus-5`。`max_plan_chars` 既定 100000
- exit code: 0 正常 / 1 `--fail-on` 該当 / 2 config・入力不正。LLM 失敗は exit code に反映しない
- コードコメントは非自明な WHY だけ。他社名・他リポジトリへの言及は書かない
- 各タスクの最後に `go build ./... && go vet ./... && go test ./...` が通ること
- コミットメッセージの末尾に `Claude-Session: https://claude.ai/code/session_01LUgt1mG81CGD4KdKkDt6Z3` を付ける

## File Structure

| path | 責務 |
| --- | --- |
| `cmd/tfreview/main.go` | cobra root、サブコマンド登録、exit code 変換 |
| `cmd/tfreview/extract.go` / `review.go` / `comment.go` / `fetch.go` | 各サブコマンドのフラグと配線だけ。ロジックは持たない |
| `internal/model/` | `Level` / `Verdict` / `Check` / `Category` と順序比較。他の全パッケージが依存する |
| `internal/plan/` | `Plan` / `Resource` 型、`terraform show -json` からの `Extract`、digest |
| `internal/config/` | YAML schema、validation、`Digest`、組み込みデフォルト（`default.yaml` を embed） |
| `internal/match/` | `match` 条件の決定的評価 |
| `internal/llm/` | `Provider` interface、`Usage`、`Pricing`、`Request` / `Response` 型 |
| `internal/llm/anthropic/` | Messages API 実装（prompt 構築・tool schema・応答パース） |
| `internal/llm/mock/` | テスト用 Provider |
| `internal/state/` | 増分キャッシュの読み書き |
| `internal/judge/` | オーケストレーション、merge、ask fallback、集約 |
| `internal/render/` | `Result` 型、`comment.md`、`result.json`、`label`、i18n 文言 |
| `internal/github/` | REST クライアント（コメント upsert、ラベル、artifact fetch） |
| `action.yml` | composite action |
| `examples/aws.yaml`, `examples/gcp.yaml` | プロバイダ固有の観点を含む完全例 |
| `testdata/` | `show-*.json` fixture、golden 出力 |
...
**Interfaces:**
- Produces: `func newRootCmd() *cobra.Command`（後続タスクがサブコマンドを `AddCommand` する）、`type exitError struct{ code int }`（`Error()` を実装。`main` が `os.Exit(code)` に変換する）

- [ ] **Step 1: module 初期化と依存追加**
...
`LICENSE` は MIT 全文、著作権者は `Masato Naka`、年は 2026。
...
`.github/workflows/ci.yml`（action の version は `gh api repos/<owner>/<repo>/releases/latest --jq .tag_name` で確認済み: checkout v7、setup-go v7、golangci-lint-action v9）:
...
**Interfaces:**
- Produces:
  - `type Level string` と定数 `LevelNone/LevelMedium/LevelHigh/LevelCritical`、`func ParseLevel(s string) (Level, error)`、`func (l Level) Rank() int`、`func MaxLevel(a, b Level) Level`、`func LevelAtLeast(l, threshold Level) bool`
  - `type VerdictKind string` と定数 `VerdictHit/VerdictMiss/VerdictUnverifiable/VerdictSkipped`、`func (v VerdictKind) Rank() int`（hit 3 > unverifiable 2 > miss 1 > skipped 0）
...
```go
// Package model は判定の語彙（レベル・verdict・チェック）を定義する。
package model
...
// skipped を最下位に置くのは、「評価できなかった」が「該当なし」より弱い情報だから。
// target 跨ぎの merge で skipped が勝ってはいけない。
...
### Task 3: internal/plan（Plan モデルと Extract）
...
**Interfaces:**
- Produces:
  - `type Counts struct { Add, Change, Destroy, Replace, Import int }`（json は `add` / `change` / `destroy` / `replace` / `import`）
  - `type Resource struct { Address, Type, Name, ModuleAddress, ProviderName string; Actions []string; After map[string]any; ChangedKeys []string }`（json は snake_case: `address` / `type` / `name` / `module_address` / `provider_name` / `actions` / `after` / `changed_keys`）
  - `type Plan struct { Target string; Counts Counts; Resources []Resource }`
  - `func Extract(showJSON []byte, target string) (*Plan, error)`
  - `func (p *Plan) HasChanges() bool`
  - `func (p *Plan) Digest() string`（`sha256:` + canonical JSON の hex）
  - `func Load(path string) (*Plan, error)` / `func (p *Plan) Save(path string) error`

`terraform show -json` の読む範囲: `resource_changes[]` の `address` / `module_address` / `type` / `name` / `provider_name` / `change.actions` / `change.before` / `change.after` / `change.after_sensitive` / `change.importing`。`actions` が `["no-op"]` / `["read"]` のものは落とす。`["delete","create"]` と `["create","delete"]` は replace。`importing` が非 null なら import に数える。
...
### Task 4: internal/config（schema、validation、digest、組み込みデフォルト）
...
- Produces:
  - `type LLM struct { Provider, Model string; MaxPlanChars int; Pricing *llm.Pricing }` … `llm.Pricing` は Task 6 で定義するので、ここでは `Pricing map[string]float64`（キー `input` / `cache_write` / `cache_read` / `output`）として持ち、Task 6 で変換する
  ...
  - `type Error struct{ Msg string }`（validation エラー。CLI が exit 2 に変換する）
  ...
  - 既定値: `Language = "en"`、`LLM.Provider = "anthropic"`、`LLM.Model = "claude-opus-5"`、`LLM.MaxPlanChars = 100000`
  - `categories` が無いとき `default.yaml`（embed）の categories を使う。`Digest` は読み込んだ生 bytes の SHA-256（デフォルト適用時は default.yaml の bytes を連結）

validation で `Error` にするもの: 未知の `level`、未知の `verdict_on_match`、`match` の未知キー、`match` の値が文字列 list でない、`question` と `match` の両方が無い、`ask` / `unverifiable` なのに `match` が無い、category / check の `id` 重複・空、`categories` が空 list、`provider` が `anthropic` / `mock` 以外。

- [ ] **Step 1: 組み込みデフォルト**

`internal/config/default.yaml`（英語。プロバイダ不問）:
...
- Produces:
  - `func Evaluate(check model.Check, plans []*plan.Plan) (model.Verdict, bool)` — `match` が無いチェックは `false` を返す。あれば必ず verdict を返す（該当なしは `miss`）
  - `func Candidates(m model.Match, p *plan.Plan) []plan.Resource` — 条件に合うリソース（`targets` が指定され p.Target が含まれなければ空）

判定規則:
- `targets` は plan の `Target` と完全一致（いずれか）
- `actions` はリソースの `Actions` のいずれかが含まれる
- `types` はリソースの `Type` がいずれかと一致
- 指定された条件すべてを満たすリソースが 1 つでもあれば該当。`targets` だけの match は、その target に resources が 1 つ以上あれば該当
- 該当時の verdict は `OnMatch` が `unverifiable` なら `VerdictUnverifiable`、それ以外（`hit` / `ask`）は `VerdictHit`。`Source` は `SourceMachine`
- `Reason` は `"<n> resource(s) matched: addr1, addr2, …"`（最大 5 件、超えたら `…and N more`）。`targets` だけの match は `"target <name> matched"`。miss は `"no resource matched"`
...
- Produces:
  ...
  - `type Answer struct { CheckID string; Kind model.VerdictKind; Reason string }`（LLM が返す生の答え。`Kind` は hit / miss / unverifiable のみ）
  ...
  - `type Pricing struct { Input, CacheWrite, CacheRead, Output float64 }`（USD / Mtok）、`var DefaultPricing = Pricing{Input: 5.00, CacheWrite: 6.25, CacheRead: 0.50, Output: 25.00}`、`func PricingFromMap(m map[string]float64) Pricing`（無いキーは Default を使う）
  - `type Provider interface { Name() string; Model() string; Judge(ctx context.Context, req Request) ([]Answer, Usage, error) }`
  - `mock.Provider`: `type Provider struct { Answers map[string][]Answer /* target → answers */; Err error; Calls []Request }`、`func (p *Provider) Judge(...)` は `Err` があれば返し、なければ `Answers[req.Plan.Target]` を返す（無い target は空）。`Usage{Calls: 1, InputTokens: 1000, OutputTokens: 100}` を返す
...
### Task 7: internal/llm/anthropic（Messages API 実装）
...
- Produces:
  - `type Options struct { Model string; MaxPlanChars int; APIKey string /* 空なら SDK が ANTHROPIC_API_KEY を読む */ }`
  - `func New(opts Options) *Provider`
  - `var ErrPlanTooLarge = errors.New("plan exceeds max_plan_chars")` — `Judge` はこれを返す。呼び出し側（judge）が全チェックを `unverifiable` にする
  - `func BuildSystem(language string) string`、`func BuildUser(req llm.Request, planJSON string) string`、`func PlanJSON(p *plan.Plan) string`（indent 2、キー順固定）
  - `func ParseAnswers(input json.RawMessage) ([]llm.Answer, error)` — tool_use の `input` をパース。未知の `verdict` はそのチェックを落とす（`skipped` になる）

呼び出しの形（SDK v1.69.0 で確認済みの型名）:
- `anthropic.NewClient(option.WithAPIKey(key))`（key が空なら option なし）
...
- 応答: `for _, block := range resp.Content { if tu, ok := block.AsAny().(anthropic.ToolUseBlock); ok { answers, err = ParseAnswers(tu.Input) } }`（`ToolUseBlock.Input` は `json.RawMessage`）
- usage: `resp.Usage.InputTokens` / `resp.Usage.CacheCreationInputTokens` / `resp.Usage.CacheReadInputTokens` / `resp.Usage.OutputTokens`（すべて `int64`）
- thinking は指定しない（Opus 5 は既定で adaptive）

システムプロンプト（`language` が `ja` なら日本語版、それ以外は英語版）:
...
- [ ] **Step 1: 失敗するテストを書く（prompt と parse）**
...
Run: `go test ./internal/llm/anthropic/`
Expected: PASS（live テストは SKIP）。コンパイルエラーが出た型名は `go doc github.com/anthropics/anthropic-sdk-go <Name>` で確認して直す
...
- Produces:
  ...
  - `func Load(path string) *State` — path が空・無い・壊れているときは空の `State` を返す（エラーにしない）
  - `func (s *State) Save(path string) error`
  - `func (s *State) Reusable(target, planDigest, configDigest string) ([]model.Verdict, bool)` — target があり `PlanDigest` と `ConfigDigest` が一致し、`skipped` を含まないときだけ返す
  - `func (s *State) Put(target, planDigest string, verdicts []model.Verdict)` — `skipped` を 1 つでも含めば書かない
...
### Task 9: internal/judge（オーケストレーション、merge、ask fallback、集約）
...
- Produces:
  ...
  - `func Run(ctx context.Context, in Input) (*Output, error)` — LLM の失敗でエラーを返さない（`skipped` にする）
  - `func Merge(vs []model.Verdict) model.Verdict`（merge.go）
  - `func Score(cfg *config.Config, verdicts map[string]model.Verdict) model.Level`、`func CategoryScore(cat model.Category, verdicts map[string]model.Verdict) model.Level`、`func IsIncomplete(verdicts map[string]model.Verdict, unevaluated map[string]bool) bool`、`func MachineScore(cfg *config.Config, verdicts map[string]model.Verdict) model.Level`（`Source == machine` の verdict だけで集約。`--fail-on-machine-only` 用）（aggregate.go）

`Run` の手順（spec §3）:
1. `len(Plans) == 0` → `NoPlans = true`、空の verdicts、`State = state.New(HeadSHA, cfg.Digest)` で返す
2. 全 plan が `!HasChanges()` → `NoChanges = true`、同様に返す
3. 各 check を `match.Evaluate`。`OnMatch == ask` かつ verdict が `miss` でないものは `askFallback[check.ID] = verdict` に控えて確定させない。それ以外で ok なものは `Verdicts` に入れ `machine[id] = true`
4. `llmChecks` = `machine` に無く `Question != ""` の check
5. target ごと: `prev.Reusable(target, plan.Digest(), cfg.Digest)` があれば再利用（`Reused = true`）。無ければ `Provider.Judge`。エラー（`ErrPlanTooLarge` 含む）または返ってこなかった check は verdict を作る: `ErrPlanTooLarge` は `unverifiable`（reason `"plan too large for LLM judgement: <err>"`）、それ以外のエラーは `skipped`（reason `"LLM judgement failed: <err>"`）、答えに無い check は `skipped`（reason `"no answer returned"`）。`Source = SourceLLM`。`llmChecks` が空なら Provider を呼ばない。得られた verdicts を `out.State.Put(target, digest, verdicts)` と `candidates[checkID]` に積む
6. `candidates` を check ごとに `Merge`
7. `Unevaluated` = candidates に `skipped` を含む check id の集合（merge 前に確定）
8. ask fallback: `askFallback` の各 check について、`Verdicts[id]` が無い・`skipped`・`Unevaluated` に含まれる、のいずれかなら `Verdicts[id]` を fallback（reason を `"LLM answer unavailable; using plan facts: " + 元 reason` に差し替え）にし `Unevaluated` から外す

`Merge`（spec §3-5）: `Rank` 最大を勝者にする。同率なら先勝ち。負けた側に `unverifiable` / `skipped` があれば勝者の reason に `" (other targets: unverifiable by plan, not evaluated)"` のように付記（種類ごとに 1 回、英語固定でよい。i18n は render で行わないため、ここは英語）。

`IsIncomplete`: `unevaluated` が空でない、または verdicts に `skipped` がある。

- [ ] **Step 1: merge / aggregate のテスト**
...
- [ ] **Step 3: merge.go / aggregate.go を実装**
...
- Create: `testdata/golden/comment-basic-en.md`, `testdata/golden/comment-basic-ja.md`（Step 4 で `-update` により生成）
...
- Produces:
  ...
  - `type Result struct { Score model.Level; MachineScore model.Level; Incomplete bool; Label string; HeadSHA string; JudgedAt string; Repo string; ConfigPath string; Language string; Model string; NoPlans, NoChanges bool; Categories []CategoryResult; Targets []TargetResult; Unevaluated []string; Usage llm.Usage; CostUSD float64 }`（json は snake_case）
  - `type Meta struct { HeadSHA, JudgedAt, Repo, ConfigPath, Model string; Pricing llm.Pricing }`
  - `func Build(cfg *config.Config, out *judge.Output, meta Meta) *Result` — `Label` は `Incomplete` なら `tfreview:unknown`、それ以外 `tfreview:<score>`。`Unevaluated` はソート済み
  - `func Comment(r *Result) string` — マーカー付き Markdown
  - `func StripBlock(body string) string` — 既存コメント本文からマーカー区間を取り除く（comment サブコマンドが使う）
...
- 見出しの絵文字: none 🟢 / medium 🟡 / high 🟠 / critical 🔴 / unknown 🔵。見出しの末尾 `— <title>` はスコアが最大の category（同率なら config 順で最初）、`none` のときは付けない。`Incomplete` のときは見出しが `## 🔵 Risk: incomplete (…)` で `Unevaluated` のチェック ID を括弧に列挙
- `NoPlans` のときは表を出さず「No plan was provided; nothing was evaluated.」、`NoChanges` のときは「No changes in any target.」+ Targets 表だけ
- `Repo` が空ならリンクにせず SHA だけ。`ConfigPath` が空なら Criteria 行を出さない
- フッターは `Usage.Calls > 0` のときだけ
- reason 内の `|` は `\|` にエスケープ、改行はスペースに

i18n: `i18n.go` に `type strings struct{...}` と `en` / `ja` の 2 セットを持ち、`Language` が `ja` のときだけ `ja`。固定文言（`Risk`、`Category`、`Hits`、`Checks`、`Targets`、`Judgement`、`reused`、`re-judged`、`Judged at`、`for`、`Criteria`、`incomplete`、`No plan was provided…`、`No changes…`、`calls`、`tokens`）を差し替える。verdict / level の語（`hit` 等）は翻訳しない（config の ID と同じ語で揃える）。
...
- Produces:
  ...
  - `func newProvider(cfg *config.Config) (llm.Provider, error)`（`provider.go`。`cfg.LLM.Provider` が `mock` なら `TFREVIEW_MOCK_ANSWERS` 環境変数の JSON（`map[target][]{check_id, verdict, reason}`）を返す mock、`anthropic` なら `anthropic.New(Options{Model, MaxPlanChars, APIKey: os.Getenv("ANTHROPIC_API_KEY")})`）
  - review の出力: `DIR/result.json`, `DIR/comment.md`, `DIR/label.txt`, `DIR/state.json`。`--out-dir` 省略時は `tfreview-out`
  - `--config` 省略時は `.tfreview.yaml`。ファイルが無ければ組み込みデフォルトで動く（`config.Parse(nil)`）。ファイルがあって壊れていれば exit 2
  - `--head-sha` 省略時は `git rev-parse HEAD` を試み、失敗したら空
  - `--fail-on` は `model.ParseLevel` で検証。該当なら `exitError{code: 1}`。`--fail-on-machine-only` のときは `MachineScore` で判定。`Incomplete` は落とさない
  - `judged_at` は `time.Now().UTC().Format(time.RFC3339)`。テストのために `now func() time.Time` をパッケージ変数にする
...
`cmd/tfreview/main.go` の `newRootCmd` に追加:
...
### Task 12: internal/github（コメント upsert、ラベル、artifact fetch）
...
- Produces:
  - `type Client struct { BaseURL string; Token string; Repo string /* owner/name */; HTTP *http.Client }`、`func New(repo, token string) *Client`（`BaseURL` 既定 `https://api.github.com`）
  - `func Token() string` — `GITHUB_TOKEN` → `GH_TOKEN` → `gh auth token` の順
  - `func (c *Client) UpsertComment(ctx, pr int, body string) error` — `GET /repos/{repo}/issues/{pr}/comments?per_page=100`（ページング）で `render.Begin` を含むコメントを探し、あれば `PATCH /repos/{repo}/issues/comments/{id}`（既存本文の block を `render.StripBlock` で除いて末尾に新 body）、なければ `POST /repos/{repo}/issues/{pr}/comments`
  - `func (c *Client) SetLabel(ctx, pr int, label string) error` — `GET /repos/{repo}/issues/{pr}/labels` で `tfreview:` 始まりを列挙し、`label` 以外を `DELETE /repos/{repo}/issues/{pr}/labels/{name}`。`label` が無ければ `POST /repos/{repo}/issues/{pr}/labels {"labels":[label]}`。404 ならラベル自体が無いので `POST /repos/{repo}/labels {"name","color": render.LabelColor(label),"description":"tfreview verdict"}` してから再度付ける。ラベル作成が 403 なら `ErrLabelForbidden` を返す（呼び出し側は警告にとどめる）
  - `func (c *Client) FetchPlanArtifact(ctx, pr int, name string, outDir string) error` — `GET /repos/{repo}/pulls/{pr}` で head sha、`GET /repos/{repo}/actions/runs?head_sha=<sha>&status=success&per_page=50`、各 run の `GET /repos/{repo}/actions/runs/{id}/artifacts` から `name` 一致の最新を選び `archive_download_url` を GET（redirect 追従）して zip を `outDir` に展開。無ければ `ErrArtifactNotFound`
  - `var ErrArtifactNotFound = errors.New("artifact not found")`、`var ErrLabelForbidden = errors.New("label creation forbidden")`

REST 呼び出しは `Authorization: Bearer <token>`、`Accept: application/vnd.github+json`、`X-GitHub-Api-Version: 2022-11-28`。テストは `httptest.Server` で `BaseURL` を差し替える。
...
- Produces:
  - `tfreview comment --result FILE --pr N --repo owner/name [--no-label]` — `result.json` を読み、`render.Comment` で本文を作って upsert、`result.Label` を付ける。`ErrLabelForbidden` は stderr に警告を出して exit 0
  - `tfreview fetch --pr N --repo owner/name --out-dir DIR [--artifact tfreview-plan]` — artifact を展開。`ErrArtifactNotFound` は exit 1 でメッセージに「no plan artifact; run terraform plan yourself or wait for CI」
  - `--repo` 省略時は `GITHUB_REPOSITORY` 環境変数
  - トークンは `github.Token()`。空なら exit 2
  - テストのために `newGitHubClient func(repo, token string) *github.Client` をパッケージ変数にし、テストは `BaseURL` を httptest に向けたものを返す
...
`zipBytes` はテストヘルパー。`cmd/tfreview/comment_test.go` の末尾に置く:
...
（import に `archive/zip` と `bytes` を足す）
...
`cmd/tfreview/main.go` の `AddCommand` を `root.AddCommand(newExtractCmd(), newReviewCmd(), newCommentCmd(), newFetchCmd())` にする。
...
### Task 14: GitHub Action（`action.yml`）とリリース（goreleaser）
...
**Interfaces:**
- Consumes: CLI の 4 サブコマンド、`tfreview-plan` artifact 名、`tfreview-state` cache key
- Produces:
  - `uses: nakamasato/tfreview@v1` の inputs: `plan-json` / `show-json` / `config` / `anthropic-api-key` / `github-token` / `fail-on` / `comment` / `version`
  - outputs: `score` / `label` / `incomplete` / `out-dir`
  - `scripts/install.sh <version> <dest>`: `https://github.com/nakamasato/tfreview/releases/download/<version>/tfreview_<os>_<arch>.tar.gz` を落として展開。`version` が `latest` なら `releases/latest/download/…`
  - リリース: tag `v*` を push すると goreleaser が linux/darwin × amd64/arm64 のバイナリを出し、`v1` のメジャータグを動かす

action version は 2026-09-02 時点の最新 major で確認済み: `actions/checkout@v7`, `actions/cache@v6`, `actions/upload-artifact@v7`, `actions/setup-go@v7`, `goreleaser/goreleaser-action@v7`。

- [ ] **Step 1: install スクリプトとテスト**
...
`scripts/install_test.sh`（URL の組み立てだけを検証。ネットワークは使わない）:
...
`cmd/tfreview/main.go` に `var version = "dev"` を足し、`root.Version = version` を設定する（`tfreview --version` が出る）。
...
### Task 15: examples と README

**Files:**
- Create: `examples/aws.yaml`, `examples/gcp.yaml`, `README.md`
- Test: `internal/config/examples_test.go`（examples が `config.Load` を通ること）

**Interfaces:**
- Consumes: `config.Load`
- Produces: ユーザー向けドキュメントと、コピーして使える config 2 本

- [ ] **Step 1: examples のテスト**
...
spec §11 の構成で書く。英語。売りは spec の順番。以下を含める:
...
括弧で示した節は spec §2 / §3 / §10 の表をそのまま英語で書く。他社名・他リポジトリへの言及が無いことを確認する。
...
- Spec coverage: §1 サブコマンド → Task 11/13、extract の出力形 → Task 3、§2 config・4 型・組み込みデフォルト → Task 4、§3 判定の流れ・増分 → Task 8/9、§4 LLM → Task 6/7、§5 出力 → Task 10、§6 comment/fetch → Task 12/13、§7 Action → Task 14、§8 パッケージ構成 → File Structure、§9 テスト → 各タスク、§11 README → Task 15
- 型の一貫性: `model.Verdict{CheckID, Kind, Reason, Source}` を全タスクで使用。`judge.Output.Targets` は `[]TargetOutcome`、`render.Build` がそれを `[]TargetResult` に変換。`llm.Usage` のフィールド名は `Calls/InputTokens/CacheWriteTokens/CacheReadTokens/OutputTokens` で統一
- 未定義参照なし: `anthropic.ErrPlanTooLarge` は Task 7 で定義し Task 9 が使う。`render.StripBlock` / `render.LabelColor` / `render.Begin` は Task 10 で定義し Task 12 が使う
